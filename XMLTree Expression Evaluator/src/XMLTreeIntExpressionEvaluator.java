...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
    	...
    	boolean result = remainder.isZero();
    	
    	//restoring original value of n
    	n.multiply(two);
    	n.add(remainder);
        return result;
    }
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        while(!p.isZero()) {
        	//If p is even
        	if(p.divide(new NaturalNumber2(2)).isZero()) {
        		base.power(2);
        		base.divide(m);
        	}else {
        	    // if p is odd
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        wSquared.power(2);
        wSquared.divide(n);
        if(wSquared.compareTo(new NaturalNumber2(1)) == 0) {
        	return true;
        }
        NaturalNumber wToNMinusOne = new NaturalNumber2(w);
        powerMod(wToNMinusOne, nMinusOne, n);
        return !wToNMinusOne.isZero() && wToNMinusOne.compareTo(nMinusOne) != 0;
    }
