...
/**
 * Update the RSS Reader program to read multiple RSS feeds from a given URL into the
    ...
    */
    private static void processFeed(String url, String file, SimpleWriter out) {
    	XMLTree rssFeed = new XMLTree1(url);
    	 // Check if the root of the XML tree is an RSS element with version 2.0.
    	if("rss".equals(rssFeed.label()) && rssFeed.hasAttribute("version") && "2.0".equals(rssFeed.attributeValue("version"))) {
    		SimpleWriter htmlOutput = new SimpleWriter1L(file);
    		XMLTree channel = rssFeed.child(0);
    		
    		outputHeader(channel, htmlOutput);
    		
    		// loop each item within the channel
    		for(int i = 0; i< channel.numberOfChildren(); i++) {
    			XMLTree child = channel.child(i);
    			// If the child is an item, process it to output its details to the HTML file.
    		...
    		out.print("The URL does not have a correct RSS feed: " + url);
    	}
    }
    ...
    public static void main(String[] args) {
        ...
        // Asking the user for name of XML file with the rss feed urls
        out.print("Enter the name of the XML file containing the RSS feed URLs: ");
        String xmlFilename = in.nextLine();
        XMLTree feedList = new XMLTree1(xmlFilename);
        // asking the user for the name of the html file
        out.print("Enter the output HTML file: ");
        String htmlFilename = in.nextLine();
        
        // Initializing the simpleWriter to write the html file
        SimpleWriter htmlOutput = new SimpleWriter1L(htmlFilename);
        // Writing the html structure of the output file with title and header
        htmlOutput.println("<html><head><title>Index Page</title></head><body>");
        htmlOutput.println("<h1>Top Stories</h1>");
        htmlOutput.println("<ul>"); // Create lists that contain the links of the feeds
        
        // Loop through each child of the feedList
        for (int i = 0; i < feedList.numberOfChildren(); i++) {
            XMLTree feed = feedList.child(i);
            // Taking the url, name and output file attributes for each feed
            ...
            // adding a list item to the index html linking to the feed file
            htmlOutput.println("<li><a href=\"" + feedFile + "\">" + feedName + "</a></li>");
        }
        // Letting the user know that its completed
        out.print("Nice work! Everything should work.");

        // end of the html for the index page
        htmlOutput.println("</ul>");
        htmlOutput.println("</body></html>");
        htmlOutput.close();
