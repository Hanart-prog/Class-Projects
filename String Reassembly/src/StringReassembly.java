...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
       ...
       //Extract the portion of str1 not involved in the overlap
       String nonOverlappingPartOfStr1 = str1.substring(0,str1.length() - overlap);
       //Concatenate the non overlapping part of the str1 with str2 which removes the overlap
       String combinedString = nonOverlappingPartOfStr1 + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet, String str) {
        ...
        Set<String> toRemove = new Set1L<>();
        for(String s : strSet) {
        	//If str contains 's', add 's' to the removal set
        	if(str.contains(s)) {
        		toRemove.add(s);
        	}else if(s.contains(str)) {
        		//If 's' contains str then don't add str to the set 
        		return;
        	}
        }
        //Remove all substrings str from strSet
        strSet.remove(toRemove);
        //Add str to the set and now all checks passed
        strSet.add(str);
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        while(!input.atEOS()) {
        	...
        	//Add line to our set and no set are substrings of each other
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //Iterate through the length of stringText
        for(int i = 0; i< stringText.length(); i++) {
        	if(stringText.charAt(i) == '~') {
        		out.println();
        	}else {
        		//Extract the current character from stringText
        		char characterPos = stringText.charAt(i);
        		out.print(characterPos);
        	}
        }
    }
